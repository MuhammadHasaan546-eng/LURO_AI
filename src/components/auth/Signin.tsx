...
const Signin = () => {
  ...
  const handleAuth = async (strategy: AuthStrategy) => {
    ...
    try {
      if (strategy === AuthStrategy.EmailPassword) {
    ...
    <div className="w-full text-white space-y-6 text-center">
      ...
      {!showEmailForm ? (
        <div className="space-y-3">
          {/* Google Button */}
          <button
            onClick={() => handleAuth(AuthStrategy.Google)}
          ...
          {/* Apple Button */}
          <button
            onClick={() => handleAuth(AuthStrategy.Apple)}
            className="w-full flex items-center justify-center gap-3 bg-white/10 hover:bg-white/20 border border-white/15 text-white font-medium py-3 px-4 rounded-xl transition duration-200 backdrop-blur-md"
          >
        ...
        <div className="space-y-4 text-left">
          <div>
            ...
            <input
              ...
              className="w-full bg-white/10 border border-white/20 rounded-xl px-4 py-3 text-white placeholder-gray-400 focus:outline-none focus:border-white transition backdrop-blur-md"
            />
          </div>

          <button
            disabled={loading}
            onClick={() => handleAuth(AuthStrategy.EmailPassword)}
            className="w-full bg-white text-black hover:bg-gray-100 font-semibold py-3 rounded-xl transition duration-200 disabled:opacity-50 shadow-lg"
          >
