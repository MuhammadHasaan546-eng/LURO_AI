...
export const SignUpPage = () => {
  ...
  // 1. Social OAuth Sign-up (Google & Apple)
  const handleSocialAuth = async (strategy: OAuthStrategy) => {
    ...
    try {
      ...
      const errorMessage = err.errors?.[0]?.message || "OAuth sign up failed";
  ...
  // 2. Direct Email Continue Flow (Passwordless / Verification)
  const handleEmailSignUp = async (e: React.FormEvent) => {
    ...
    try {
      // Initiate sign-up with email and optional names
      ...
      if (result.status === "complete") {
        ...
        // Prepare email code verification (Clerk Passwordless)
      ...
      const errorMessage =
        err.errors?.[0]?.message || "Failed to sign up with email";
      setError(errorMessage);
      toast.error(errorMessage);
    } finally {
      setLoading(false);
