...
type SignUpProps = { oauthError?: string };

export const SignUpPage = ({ oauthError }: SignUpProps) => {
  ...
  return (
    <div className="w-full text-white space-y-6 text-center">
      ...
      {(oauthError || formErrors.length > 0 || error) && (
        <div
          role="alert"
      ...
      <div className="grid grid-cols-2 gap-3">
        <Link
          prefetch={false}
          href="/api/auth/oauth/google?intent=signup"
        ...
        <Link
          prefetch={false}
          href="/api/auth/oauth/apple?intent=signup"
          className="inline-flex items-center justify-center gap-2 rounded-xl border border-white/15 bg-white/5 px-3 py-3 text-sm font-medium hover:bg-white/10 transition duration-200"
        >
